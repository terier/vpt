// #part /glsl/shaders/renderers/ISOConductor/generate/vertex
...
// #part /glsl/shaders/renderers/ISOConductor/generate/fragment
...
// #part /glsl/shaders/renderers/ISOConductor/integrate/vertex
...
// #part /glsl/shaders/renderers/ISOConductor/integrate/fragment
...
// #part /glsl/shaders/renderers/ISOConductor/render/vertex
...
// #part /glsl/shaders/renderers/ISOConductor/render/fragment
...
vec3 sampleGradient(vec3 position, float h) {
    vec3 positive = vec3(
        sampleVolume(position + vec3( h, 0, 0)),
        sampleVolume(position + vec3(0,  h, 0)),
        sampleVolume(position + vec3(0, 0,  h))
    );
    vec3 negative = vec3(
        sampleVolume(position + vec3(-h, 0, 0)),
        sampleVolume(position + vec3(0, -h, 0)),
...
void main() {
    ...
    if (closest.w > 0.0) {
        vec3 normal = normalize(sampleGradient(closest.xyz, 0.005));
        vec3 light = normalize(uLight);
        float lambert = max(dot(normal, light), 0.0);
        oColor = vec4(uDiffuse * lambert, 1);
    } else {
        oColor = vec4(1);
    }
}

// #part /glsl/shaders/renderers/ISOConductor/reset/vertex
...
// #part /glsl/shaders/renderers/ISOConductor/reset/fragment
